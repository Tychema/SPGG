    ...
    for i in tqdm(range(epoches), desc='Processing'):
        ...
        if i != 0:
            ...
            if random_number < 0.5:
                ...
                Q2_table=updateQMatrix(type_t_minus_matrix, type_t_matrix, Q2_table,Q_table, profit_matrix)
                update_Q_table = Q2_table
        # 博弈演化,type变换，策略传播
        type_t1_matrix = type_matrix_change(type_t_matrix, update_Q_table).to(device)
        # 把一个L的三个type分开
