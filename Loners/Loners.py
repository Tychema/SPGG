...
l_profit_matrix = torch.full((L_num, L_num),eta*(r-1),dtype=torch.float16).to(device)
...
def updateQMatrix(type_t_matrix: tensor, type_t1_matrix: tensor, Q_tensor: tensor, profit_matrix: tensor):
    ...
    update_values = Q_tensor[C_indices, A_indices, B_indices] + alpha * (
                profit_matrix.view(-1) + gamma * max_values - Q_tensor[C_indices, A_indices, B_indices])
    # print(update_values)
    # 更新 type_t_matrix
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(epoches), desc='Processing'):
        ...
        if (i + 1) % 10000 == 0:
            ...
            plt.plot(obsX, D_Y, 'ro', label='betray', linestyle='-', linewidth=1, markeredgecolor='b', markersize='1',
                     markeredgewidth=1)
            plt.plot(obsX, C_Y, 'bo', label='betray', linestyle='-', linewidth=1, markeredgecolor='r',
                     markersize='1', markeredgewidth=1)
            plt.plot(obsX, L_Y, 'yo', label='loners', linestyle='-', linewidth=1, markeredgecolor='g',
                     markersize='1', markeredgewidth=1)
            ...
            plt.plot(obsX, D_Value, 'ro', label='betray', linestyle='-', linewidth=1, markeredgecolor='b',
                     markersize='1', markeredgewidth=1)
            plt.plot(obsX, C_Value, 'bo', label='betray', linestyle='-', linewidth=1, markeredgecolor='r',
                     markersize='1', markeredgewidth=1)
            plt.plot(obsX, L_Value, 'yo', label='loners', linestyle='-', linewidth=1, markeredgecolor='g',
                     markersize='1', markeredgewidth=1)
            plt.xscale('log')
