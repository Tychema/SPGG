...
def calculation_value2(W_matrix: tensor, d_matrix, c_matrix, r_matrix):
    ...
    profit_matrix = c_matrix * ((coorperation_num) / g_matrix * r - 1 + u / g_matrix * w_num) + d_matrix * (
        coorperation_num) / g_matrix * r + r_matrix * ((
                                                    coorperation_num) / g * r - 1 + u / g_matrix * w_num - alpha * u / g_matrix * W_matrix * coorperation_num)
...
def calculation_value(W_matrix: tensor, d_matrix, c_matrix, r_matrix):
    with torch.no_grad():
        ...
        # c和r最后的-1是最开始要贡献到池里面的1
        c_profit_matrix = (coorperation_num) / g_matrix * r - 1 + u / g_matrix * w_num

        d_profit_matrix = (coorperation_num) / g_matrix * r
        r_profit_matrix = (coorperation_num) / g_matrix * r - 1 + u / g_matrix * w_num - alpha * u / g_matrix * W_matrix * coorperation_num
...
def cal_w(type_t_matrix: tensor, type_t1_matrix: tensor, w_matrix: tensor, r_matrix: tensor):
    ...
    conv_result = torch.nn.functional.conv2d(input=update_condition.view(1, 1, L_num, L_num).to(torch.float16),
                                             weight=new_kernel, padding=2, stride=1, bias=None).to(torch.float16).to(device)
