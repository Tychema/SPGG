#用Qlearning实现Fermi博弈
#rand(0,exp(7.007*profit_matrix))方法
...
class SPGG_Qlearning(nn.Module):
    ...
    def updateQMatrix(self,type_t_matrix: tensor, profit_matrix: tensor):
        ...
        # 获取float64的最大值
        max_value = torch.finfo(torch.float64).max
        # 将无穷值替换为float64的最大值
        rand_rate = torch.rand_like(profit_matrix.to(torch.float64),dtype=torch.float64)
        exp_profit=torch.exp(7.007 * profit_matrix).to(torch.float64)
        exp_profit[torch.isinf(exp_profit)] = max_value
        ...
        profit_left,profit_right,profit_up,profit_down=self.indices_Matrix_to_Four_Matrix(profit_matrix)
        #生成一个矩阵随机决定向哪个方向学习
        ...
        exp_learning_profit=torch.exp(7.007 * learning_profit_t).to(torch.float64)
        exp_learning_profit[torch.isinf(exp_learning_profit)] = max_value
    ...
    def shot_pic(self,type_t_matrix: tensor,i,r):
        ...
        self.mkdir('data/Origin_Qlearning_Fermi/shot_pic/r={}/two_type/generated1'.format(r))
        plt.savefig('data/Origin_Qlearning_Fermi/shot_pic/r={}/two_type/generated1/t={}.png'.format(r,i))
        self.mkdir('data/Origin_Qlearning_Fermi/shot_pic/r={}/two_type/generated1/type_t_matrix'.format(r))
        np.savetxt('data/Origi_Qlearningn_Fermi/shot_pic/r={}/two_type/generated1/type_t_matrix/{}_r={}_epoches={}_L={}_第{}次实验数据.txt'.format(str(r),"type_t_matrix",str(r),str(self.epoches),str(self.L_num),str(self.count)),type_t_matrix)
        #plt.show()
        plt.clf()
        plt.close("all")
    ...
    def shot_pic2(self,type_t_matrix: tensor,i,r):
        ...
        plt.title('Qlearning: '+f"T:{i}")
        plt.imshow(image,interpolation='None')
        self.mkdir('data/Origin_Qlearning_Fermi/shot_pic/r={}/four_type/generated1'.format(r))
        plt.savefig('data/Origin_Qlearning_Fermi/shot_pic/r={}/four_type/generated1/t={}.png'.format(r,i))
        self.mkdir('data/Origin_Qlearning_Fermi/shot_pic/r={}/four_type/generated1/type_t_matrix'.format(r))
        np.savetxt('data/Origin_Qlearning_Fermi/shot_pic/r={}/four_type/generated1/type_t_matrix/{}_r={}_epoches={}_L={}_第{}次实验数据.txt'.format(str(r),"type_t_matrix",str(r),str(self.epoches),str(self.L_num),str(self.count)),type_t_matrix)
        #plt.show()
        plt.clf()
        plt.close("all")
    #计算CD比例和利润
    ...
    def run(self,r,alpha,gamma,epsilon,epoches, L_num, device,type):
        ...
        for i in tqdm(range(epoches), desc='Processing'):
            ...
            Q_matrix = self.updateQMatrix(type_t_matrix, profit_matrix)
    ...
    def save_data(self,type,name,r,count,data):
        self.mkdir('data/Origin_Qlearning_Fermi/'+str(type))
        np.savetxt('data/Origin_Qlearning_Fermi/{}/{}_r={}_epoches={}_L={}_第{}次实验数据.txt'.format(str(type), name,str(r),str(self.epoches),str(self.L_num),str(count)), data)
        # try:
    ...
    def run_line2_pic(self,loop_num1=51,loop_num2 = 10):
        ...
        for j in range(loop_num1):
            for i in range(loop_num2):
                ...
                CC_data, DD_data, CD_data, DC_data, DD_Y, CC_Y, CDC_Y, StickStrategy_Y, DD_value_np, CC_value_np, CDC_value_np, StickStrategy_value_np = self.run(
...
def draw_shot():
    r_list=[4.0]
    for r in r_list:
        SPGG = SPGG_Qlearning(L_num, device, alpha, gamma, epsilon,lr=0.2, r=r, epoches=10000, cal_transfer=True)
        SPGG.line1_pic(r)

if __name__ == '__main__':
    r=3.8
    SPGG=SPGG_Qlearning(L_num,device,alpha,gamma,epsilon,r=r,epoches=10000,lr=0.2,cal_transfer=True)
