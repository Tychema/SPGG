...
class SPGG_Qlearning(nn.Module):
    def __init__(self,L_num,device,alpha,gamma,epsilon,r,epoches,lr=0.2,eta=0.8,count=0,cal_Q=False):
    ...
    def run(self,r,alpha,gamma,epsilon,epoches, L_num, device,type):
        ...
        convergence_threshold = 0.005  # 可以根据需要调整收敛阈值
        # r_threshold = 0.99  # 可以根据需要调整收敛阈值
        window_size = 10000  # 方差计算的窗口大小
        ...
        for i in tqdm(range(epoches), desc='Processing'):
            ...
            D_Y, C_Y, D_Value, C_Value, count_0, count_1 = self.cal_fra_and_value(
                D_Y, C_Y, D_Value, C_Value, type_t_matrix, d_matrix, c_matrix, profit_matrix)
            Q_DD, Q_DC, Q_CD, Q_CC = np.array([]), np.array([]), np.array([]), np.array([])
            C_Y_window.append(C_Y[-1])
            if self.cal_Q:
                Q_DD = np.append(Q_DD, Q_matrix[:, 0, 0].mean().cpu().numpy())
                Q_DC = np.append(Q_DC, Q_matrix[:, 0, 1].mean().cpu().numpy())
                Q_CD = np.append(Q_CD, Q_matrix[:, 1, 0].mean().cpu().numpy())
                Q_CC = np.append(Q_CC, Q_matrix[:, 1, 1].mean().cpu().numpy())
            if i > window_size:  # 当窗口达到 window_size 时，移除最早的数据，保持窗口大小不变
                C_Y_window.pop(0)
                # 计算均值
                mean = np.mean(C_Y_window)
                # 计算方差
                variance = np.var(C_Y_window)

                # 如果均值大于0.9且方差小于收敛阈值，且迭代次数大于2000，则认为收敛
                if (
                        # mean >= r_threshold and
                        variance < convergence_threshold and i >= 5000):
                    print(f'variance: {variance:.6f}' + f' Converged at epoch {i + 1}')
                    iterations = i + 1  # 因为索引是从0开始的，所以需要加1
                    break

        if self.cal_Q:
            return D_Y, C_Y, D_Value, C_Value, Q_DD, Q_DC, Q_CD, Q_CC
    ...
    def save_data(self,type,name,r,count,data):
        self.mkdir('data/Origin_Fermi_Qlearning2/heat_pic3_T/eta={}/gammm={}/'.format(str(self.eta),str(self.gamma))+str(type))
        np.savetxt('data/Origin_Fermi_Qlearning2/heat_pic3_T/eta={}/gammm={}/{}/{}_r={}_epoches={}_L={}_第{}次实验数据.txt'.format(str(self.eta),str(self.gamma),str(type), name,str(r),str(self.epoches),str(self.L_num),str(count)), data)


    def line1_pic(self, r,i):
        self.count = i
        if self.cal_Q:
            D_Y, C_Y, D_Value, C_Value, Q_DD, Q_DC, Q_CD, Q_CC= self.run(self.r, self.alpha, self.gamma, self.epsilon, self.epoches, self.L_num, self.device, type="line1")
        else:
        ...
        self.save_data('iretations', 'iretations', r, i, np.array([C_Y.shape]))
        print(np.mean(C_Y[-100:]))
        if self.cal_Q:
            self.save_data('Q_DD', 'Q_DD', r, i, Q_DD)
            self.save_data('Q_DC', 'Q_DC', r, i, Q_DC)
            self.save_data('Q_CD', 'Q_CD', r, i, Q_CD)
            self.save_data('Q_CC', 'Q_CC', r, i, Q_CC)
    ...
    for i in range(loop_num1):
        for eta1 in range(loop_num_eta):
            for gamma1 in range(loop_num_gamma):
                ...
                print("eta=", eta2," gamma=", gamma2, " 第i轮: ", i)
                SPGG = SPGG_Qlearning(50, device, alpha, gamma2, epsilon, lr=0.2, r=r, epoches=1000000, eta=eta2, cal_Q=True)
                SPGG.line1_pic(r,i)

def buquan_heat_pic(cal_Q=False):
    r=2.9
    loop_num1=1
    loop_num_eta=1
    loop_num_gamma=1
    ...
    for i in range(loop_num1):
        for eta1 in range(loop_num_eta):
            for gamma1 in range(loop_num_gamma):
                ...
                gamma2=(start_gamma+gamma1)/100
                print("eta=", eta2," gamma=", gamma2, " 第i轮: ", i)
                SPGG = SPGG_Qlearning(50, device, alpha, gamma2, epsilon, lr=0.2, r=r, epoches=1000000, eta=eta2, cal_Q=cal_Q)
                SPGG.line1_pic(r,i)

def buquan_zero():
    #读取"data/Origin_Fermi_Qlearning2/zero.csv"文件
    with open("data/Origin_Fermi_Qlearning2/heat_pic3_T/zero.csv", "r") as f:
        lines = f.readlines()
    #第一列为gamma，第二列为eta
    for line in lines:
        gamma, eta = line.strip().split(",")
        print("eta="+ str(int(eta)/100)," gamma=", str(int(gamma)/100), " 第i轮: ", str(0))
        SPGG = SPGG_Qlearning(50, device, alpha, int(gamma)/100, epsilon, lr=0.2, r=2.9, epoches=1000000, eta=int(eta)/100,cal_Q=False)
        SPGG.line1_pic(2.9,0)
