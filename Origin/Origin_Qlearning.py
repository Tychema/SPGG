...
class SPGG_Qlearning(nn.Module):
    ...
    def shot_pic(self,type_t_matrix: tensor,i,r):
        ...
        plt.imshow(image)
        self.mkdir('data/Origin_Qlearning/shot_pic/r={}/two_type/generated2'.format(r))
        plt.savefig('data/Origin_Qlearning/shot_pic/r={}/two_type/generated2/t={}.png'.format(r,i))
        plt.show()
        plt.clf()
        plt.close("all")
    ...
    def shot_pic2(self,type_t_matrix: tensor,i,r):
        ...
        plt.title('Qlearning: '+f"T:{i}")
        plt.imshow(image)
        self.mkdir('data/Origin_Qlearning/shot_pic/r={}/four_type/generated2'.format(r))
        plt.savefig('data/Origin_Qlearning/shot_pic/r={}/four_type/generated2/t={}.png'.format(r,i))
        plt.show()
        plt.clf()
        plt.close("all")
    #计算CD比例和利润
    ...
    def cal_four_type_value(self,DD,CC,CDC,StickStrategy,profit_matrix):
        ...
        return CC_value, DD_value, CDC_value, StickStrategy_value
    ...
    def run(self,r,alpha,gamma,epsilon,epoches, L_num, device,type):
        ...
        type_t_matrix = self.generated_default_type_matrix2().to(device)
        ...
        CC_value_np, DD_value_np, CDC_value_np, StickStrategy_value_np = np.array([]), np.array([]), np.array([]), np.array([])
        ...
        for i in tqdm(range(epoches), desc='Processing'):
            ...
            CC_value, DD_value, CDC_value, StickStrategy_value = self.cal_four_type_value( DD,CC, CDC, StickStrategy,profit_matrix)
        ...
        if (type == "line2"):
            ...
            return D_Y, C_Y, D_Value, C_Value,all_value, Q_matrix, type_t_matrix, count_0, count_1,  CC_data, DD_data, CD_data, DC_data,DD_Y,CC_Y,CDC_Y,StickStrategy_Y,DD_value_np,CC_value_np,CDC_value_np,StickStrategy_value_np
        elif (type == "Qtable"):
    ...
    def save_data(self,type,name,r,count,data):
        self.mkdir('data/Origin_Qlearning/generated2/'+str(type))
        try:
            np.savetxt('data/Origin_Qlearning/generated2/{}/{}_r={}_epoches={}_L={}_第{}次实验数据.txt'.format(str(type), name, str(r),str(self.epoches),str(self.L_num), str(count)),data)
        except:
            print("Save failed")
    ...
    def run_line2_pic(self,loop_num1=50,loop_num2 = 10):
        ...
        for j in range(loop_num1):
            for i in range(loop_num2):
                ...
                D_Y, C_Y, D_Value, C_Value,all_value, Q_matrix, type_t_matrix, count_0, count_1,  CC_data, DD_data, CD_data, DC_data,DD_Y,CC_Y,CDC_Y,StickStrategy_Y,DD_value_np,CC_value_np,CDC_value_np,StickStrategy_value_np = self.run(
    ...
    def cal_transfer_pic(self):
        D_Y, C_Y, D_Value, C_Value,all_value, Q_matrix, type_t_matrix, count_0, count_1,  CC_data, DD_data, CD_data, DC_data=self.run(self.r, self.alpha,self.gamma,self.epsilon,self.epoches, self.L_num,self.device,type="line1")
    ...
    def line1_pic(self, r):
        ...
        DD_Y_ave, CC_Y_ave, CDC_Y_ave, StickStrategy_Y_ave, DD_value_np_ave, CC_value_np_ave, CDC_value_np_ave, StickStrategy_value_np_ave=np.zeros(epoches),np.zeros(epoches),np.zeros(epoches),np.zeros(epoches),np.zeros(epoches),np.zeros(epoches),np.zeros(epoches),np.zeros(epoches)
        for i in range(loop_num):
            print("第i轮:", i)
            D_Y, C_Y, D_Value, C_Value,all_value, Q_matrix, type_t_matrix, count_0, count_1,  CC_data, DD_data, CD_data, DC_data,DD_Y,CC_Y,CDC_Y,StickStrategy_Y,DD_value_np,CC_value_np,CDC_value_np,StickStrategy_value_np = self.run(
